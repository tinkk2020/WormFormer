...
#tenghui
@TRANSFORMS.register_module()
class PackDetInputs_localFeature(BaseTransform):
    """Pack the inputs data for the detection / semantic segmentation /
    panoptic segmentation.
    ...
    mapping_table = {
        ...
        'gt_bboxes_labels': 'labels',
        'gt_masks': 'masks',
        'gt_localFeatures':'localFeatures'
    }
    ...
    def transform(self, results: dict) -> dict:
        ...
        for key in self.mapping_table.keys():
            ...
            if key == 'gt_masks' or isinstance(results[key], BaseBoxes):
                ...
                if 'gt_ignore_flags' in results:
                    # results[key][valid_idx] : only integer scalar arrays can be converted to a scalar index
                    if key == 'gt_localFeatures':
                        instance_loacFeatures = []
                        for index in valid_idx:
                            instance_loacFeatures.append(results['gt_localFeatures'][index])
                        instance_data[self.mapping_table[key]] = instance_loacFeatures
                        ignore_instance_loacFeatures = []
                        for ignore_index in ignore_idx:
                            ignore_instance_loacFeatures.append(results['gt_localFeatures'][ignore_index])
                        ignore_instance_data[self.mapping_table[key]] = ignore_instance_loacFeatures
                    else:
                        instance_data[self.mapping_table[key]] = to_tensor(
                            results[key][valid_idx])
                        ignore_instance_data[self.mapping_table[key]] = to_tensor(
                    ...
                    if key == 'gt_localFeatures':
                        instance_data[self.mapping_table[key]] = results['gt_localFeatures']
                    else:
                        instance_data[self.mapping_table[key]] = to_tensor(
                            results[key])
        data_sample.gt_instances = instance_data
        ...
        # cv2.imwrite('/home/poac/disk16T/dataset/newdataBBBC/fortest/pack32.png', showimg)
        return packed_results
...
#liye
@TRANSFORMS.register_module()
class PackDetInputs_SketchPoints(BaseTransform):
    """Pack the inputs data for the detection / semantic segmentation /
    panoptic segmentation.
            ...
            if key == 'gt_masks' or isinstance(results[key], BaseBoxes):
                ...
                if 'gt_ignore_flags' in results:
                    ...
                    if key == 'gt_SketchPoints':
                        ...
                        #print(np.isinf(results[key]).max()+np.isnan(results[key]).max())
                    ...
                    if key == 'gt_SketchPoints':
                        instance_data[self.mapping_table[key]] = results['gt_SketchPoints']
                    else:
                        instance_data[self.mapping_table[key]] = to_tensor(
                            results[key])
        data_sample.gt_instances = instance_data
