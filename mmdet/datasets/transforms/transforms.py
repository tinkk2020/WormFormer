...
class RandomResize_localFeature(BaseTransform):
    ...
    def __init__(
        ...
        resize_type: str = 'Resize_localFeature',
        **resize_kwargs,
    ) -> None:
        ...
        self.resize_cfg = dict(type='Resize_localFeature', **resize_kwargs)
    ...
    def transform(self, results: dict) -> dict:
        ...
        # import cv2
        # showimg = results['img']
        # for gt_localFeature in results['gt_localFeatures']:
        #     centerline = gt_localFeature[0]
        #     drawlinex = centerline[0::2]
        #     drawliney = centerline[1::2]
        #     for i in range(len(drawlinex)):
        #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 0, 255), -1)
        # cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/resize'+results['img_path'].split('/')[-1], showimg)
        return results
...
#tenghui
@TRANSFORMS.register_module()
class Resize_localFeature(MMCV_Resize):
    ...
    def _resize_localFeatures(self, results: dict) -> None:
        """Resize bounding boxes with ``results['scale_factor']``."""
        if results.get('gt_localFeatures', None) is not None:
            localFeature32 = []
            localFeature64 = []
            localFeature128 = []
            gt_localFeatures = []
            for localFeature_all in results['gt_localFeatures']:
                localFeature32.append(localFeature_all[0])
                localFeature64.append(localFeature_all[1])
                localFeature128.append(localFeature_all[2])
            localFeature32 = torch.tensor(np.array(localFeature32, dtype='float32'))
            localFeature64 = torch.tensor(np.array(localFeature64, dtype='float32'))
            localFeature128 = torch.tensor(np.array(localFeature128, dtype='float32'))
            localFeature32 = rescale_localFeature(localFeature32,results['scale_factor'])
            localFeature64= rescale_localFeature(localFeature64,results['scale_factor'])
            localFeature128 = rescale_localFeature(localFeature128, results['scale_factor'])
            if self.clip_object_border:
                localFeature32 = clip_localFeature(localFeature32, results['img_shape'])
                localFeature64 = clip_localFeature(localFeature64, results['img_shape'])
                localFeature128 = clip_localFeature(localFeature128, results['img_shape'])
            for i in range(len(results['gt_localFeatures'])):
                one_instance = []
                one_instance.append(localFeature32[i])
                one_instance.append(localFeature64[i])
                one_instance.append(localFeature128[i])
                gt_localFeatures.append(one_instance)
            results['gt_localFeatures'] = gt_localFeatures
            ...
            #     drawliney = centerline[1::2]
            #     for i in range(len(drawlinex)):
            #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (255, 0, 0), -1)
            # cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/resize32.png', showimg)
            # print('liye debug')
    ...
    def transform(self, results: dict) -> dict:
        ...
        self._resize_localFeatures(results)
...
#liye, for sketch points
@TRANSFORMS.register_module()
class RandomResize_SketchPoints(BaseTransform):
    """Random resize images & bbox & keypoints.
    ...
    def __init__(
        ...
        scale: Union[Tuple[int, int], Sequence[Tuple[int, int]]],
        ratio_range: Tuple[float, float] = None,
        resize_type: str = 'Resize_SketchPoints',
        **resize_kwargs,
    ) -> None:
        ...
        self.resize_cfg = dict(type='Resize_SketchPoints', **resize_kwargs)
        # create a empty Reisize object
        self.resize = TRANSFORMS.build({'scale': 0, **self.resize_cfg})
    ...
    def transform(self, results: dict) -> dict:
        ...
        # import cv2
        # showimg = results['img']
        # for centerline in results['gt_SketchPoints']:
        #     drawlinex = centerline[0::2]
        #     drawliney = centerline[1::2]
        #     for i in range(len(drawlinex)):
        #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (255,0,0), -1)
...
#liye, for sketch points
@TRANSFORMS.register_module()
class Resize_SketchPoints(MMCV_Resize):
    """Resize images & bbox & seg.
    ...
    def _resize_localFeatures(self, results: dict) -> None:
        """Resize bounding boxes with ``results['scale_factor']``."""
        if results.get('gt_SketchPoints', None) is not None:
            gt_localFeatures = results['gt_SketchPoints']
            gt_localFeatures = torch.tensor(np.array(gt_localFeatures, dtype='float32'))
            gt_localFeatures = rescale_localFeature(gt_localFeatures, results['scale_factor'])
            if self.clip_object_border:
                gt_localFeatures = clip_localFeature(gt_localFeatures, results['img_shape'])
            results['gt_SketchPoints'] = gt_localFeatures

            # #for test
            # import cv2
            # showimg = results['img']
            # for gt_localFeature in results['gt_SketchPoints']:
            #     centerline = gt_localFeature
            #     drawlinex = centerline[0::2]
            #     drawliney = centerline[1::2]
            #     for i in range(len(drawlinex)):
            #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 0, 255), -1)
...
#tenghui
@TRANSFORMS.register_module()
class RandomFlip_localFeature(MMCV_RandomFlip):
    """Flip the image & bbox & mask & segmentation map. Added or Updated keys:
    flip, flip_direction, img, gt_bboxes, and gt_seg_map. There are 3 flip
    modes:
    ...
    def _flip(self, results: dict) -> None:
        ...
        # tenghui flip centerines
        # we need pack the same lenth localFeature,process,then unpack?
        if results.get('gt_localFeatures', None) is not None:
            localFeature32 = []
            localFeature64 = []
            localFeature128 = []
            ...
            localFeature64 = torch.tensor(np.array(localFeature64, dtype='float32'))
            localFeature128 = torch.tensor(np.array(localFeature128, dtype='float32'))
            localFeature32 = flip_localFeature(localFeature32,img_shape, results['flip_direction'])
            localFeature64 =flip_localFeature(localFeature64,img_shape, results['flip_direction'])
            localFeature128 = flip_localFeature(localFeature128,img_shape, results['flip_direction'])
            for i in range(len(results['gt_localFeatures'])):
                one_instance = []
                one_instance.append(localFeature32[i])
                one_instance.append(localFeature64[i])
        ...
        # # for test
        # import cv2
        # showimg = np.array(results['img'],dtype='uint8')
        # for gt_localFeature in results['gt_localFeatures']:
        #     centerline = gt_localFeature[0]
        #     drawlinex = centerline[0::2]
        #     drawliney = centerline[1::2]
        #     for i in range(len(drawlinex)):
        #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 0, 255), -1)
        # cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/flip32.png', showimg)
        # print('liye debug!!')

#liye, for sketch points
@TRANSFORMS.register_module()
class RandomFlip_SketchPoints(MMCV_RandomFlip):
    """Flip the image & bbox & mask & segmentation map. Added or Updated keys:
    flip, flip_direction, img, gt_bboxes, and gt_seg_map. There are 3 flip
    modes:
    ...
    def _flip(self, results: dict) -> None:
        ...
        # we need pack the same lenth localFeature,process,then unpack?
        if results.get('gt_SketchPoints', None) is not None:
            gt_localFeatures = torch.tensor(np.array(results['gt_SketchPoints'], dtype='float32'))
            gt_localFeatures = flip_localFeature(gt_localFeatures, img_shape, results['flip_direction'])
            results['gt_SketchPoints'] = gt_localFeatures
        ...
        #     for i in range(len(drawlinex)):
        #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 0, 255), -1)
        # cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/flip'+results['img_path'].split('/')[-1], showimg)
        #print('liye debug!!')
...
#
#         ## get x, y
#         localFeatures_pred_size = localFeatures_pred.shape[-1]
#         localFeatures_pred_x = localFeatures_pred[:,:, 0::1]
#         localFeatures_pred_y = localFeatures_pred[:, :, 1::1]
#
#
...
#
#         ## compute the number of added points for each line
#         upsample_interval = int((localFeatures_outSize-localFeatures_pred_size)/(localFeatures_pred_size-1))
#         upsample_interval_lastPoint = (localFeatures_outSize-localFeatures_pred_size)%(localFeatures_pred_size-1)
#
#         ## add points
#         localFeatures_pred_x_add_list = []
#         localFeatures_pred_y_add_list = []
#         for i in range(upsample_interval):
#             localFeatures_pred_x_add = localFeatures_pred_x[:, :, 0:-2] + localFeatures_pred_x_diff[:, :, 0:-1] * (i+1) / (upsample_interval+1)
#             localFeatures_pred_y_add = localFeatures_pred_x_add * localFeatures_pred_slope[:, :, 0:-1]
...
#
#         ## add points for the last line
#         for i in range(upsample_interval+upsample_interval_lastPoint):
#             localFeatures_pred_x_add_last = localFeatures_pred_x[:, :, -2] + localFeatures_pred_x_diff[:,:,-1] * (i+1) / (upsample_interval+upsample_interval_lastPoint+1)
#             localFeatures_pred_y_add_last = localFeatures_pred_x_add * localFeatures_pred_slope[:,:,-1]
#
#         ## put all points in order
#         localFeatures_pred_up_x = torch.zeros((localFeatures_pred.shape[0],localFeatures_pred.shape[1],int(localFeatures_outSize/2)))
#         localFeatures_pred_up_y = torch.zeros((localFeatures_pred.shape[0],localFeatures_pred.shape[1],int(localFeatures_outSize/2)))
#         localFeatures_pred_up_x[:,:,0:-1*(upsample_interval+upsample_interval_lastPoint+1):upsample_interval+1] = localFeatures_pred_x
#         localFeatures_pred_up_y[:,:,0:-1 * (upsample_interval + upsample_interval_lastPoint + 1):upsample_interval + 1] = localFeatures_pred_y
#         for i in range(upsample_interval):
#             localFeatures_pred_up_x[:,:,(i+1):(-1 * (
#                         upsample_interval + upsample_interval_lastPoint + 1)-1):(upsample_interval + 1)] = \
#                 localFeatures_pred_x_add_list[i]
#             localFeatures_pred_up_y[:,:,(i+1): (-1 * (
#                     upsample_interval + upsample_interval_lastPoint + 1) - 1):(upsample_interval + 1)] = \
#             localFeatures_pred_y_add_list[i]
#
#         localFeatures_pred_up_x[:,:,-1*(upsample_interval+upsample_interval_lastPoint+1):] = localFeatures_pred_x_add_last
#         localFeatures_pred_up_y[:, :,-1 * (upsample_interval + upsample_interval_lastPoint + 1):] = localFeatures_pred_y_add_last
#
#         localFeatures_pred_up = torch.zeros(
#             (localFeatures_pred.shape[0], localFeatures_pred.shape[1], localFeatures_outSize))
#
#         ## output the upsampled points
#         localFeatures_pred_up[:,:, 0::1] = localFeatures_pred_up_x
#         localFeatures_pred_up[:, :, 1::1] = localFeatures_pred_up_y
#
...
#tenghui
@TRANSFORMS.register_module()
class RandomCrop_localFeature(BaseTransform):
    """Random crop the image & bboxes & masks.
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        #tenghui
        # crop localFeatures  accordingly and clip to the image boundary
        if results.get('gt_localFeatures', None) is not None:
            localFeature32 = []
            localFeature64 = []
            localFeature128 = []
            ...
            localFeature64 = torch.tensor(np.array(localFeature64, dtype='float32'))
            localFeature128 = torch.tensor(np.array(localFeature128, dtype='float32'))
            localFeature32=translate_localFeature(localFeature32,[-offset_w, -offset_h])
            localFeature64=translate_localFeature(localFeature64,[-offset_w, -offset_h])
            localFeature128=translate_localFeature(localFeature128,[-offset_w, -offset_h])

            for i in range(len(valid_inds)):
                one_instance=[]
                if valid_inds[i]:#True
                    one_instance_32 = is_inside_and_conunt_localFeature(localFeature32[i], img_shape[:2])
                    if len(one_instance_32) == 0:
                        valid_inds[i] = False
                        continue
                    one_instance_32 = local_feature_upsample(one_instance_32, 32)
                    one_instance_64 = is_inside_and_conunt_localFeature(localFeature64[i], img_shape[:2])
                    one_instance_64 = local_feature_upsample(one_instance_64, 64)
                    one_instance_128 = is_inside_and_conunt_localFeature(localFeature128[i], img_shape[:2])
                    one_instance_128 = local_feature_upsample(one_instance_128, 128)
                    one_instance.append(one_instance_32)
                    one_instance.append(one_instance_64)
                    one_instance.append(one_instance_128)
                    gt_localFeatures.append(one_instance)
            results['gt_localFeatures'] = gt_localFeatures
            results['gt_bboxes'] = bboxes[valid_inds]#renew valid_inds
 #------
            if results.get('gt_ignore_flags', None) is not None:
                results['gt_ignore_flags'] = \
                    results['gt_ignore_flags'][valid_inds]
       ...
       #      drawliney = centerline[1::2]
       #      for i in range(len(drawlinex)):
       #          showimg = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 0, 255), -1)
       #  cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/crop32.png', showimg)
        return results
...
#liye, for sketch points
@TRANSFORMS.register_module()
class RandomCrop_SketchPoints(BaseTransform):
    """Random crop the image & bboxes & masks.
    ...
    def __init__(self,
        ...
        self.bbox_clip_border = bbox_clip_border
        self.recompute_bbox = recompute_bbox
        self.num_sketchPoints = num_sketchPoints
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        # liye, crop localFeatures accordingly
        if results.get('gt_SketchPoints', None) is not None:
            gt_localFeatures = results['gt_SketchPoints']
            gt_localFeatures = torch.tensor(np.array(gt_localFeatures, dtype='float32'))
            gt_localFeatures = translate_localFeature(gt_localFeatures, [-offset_w, -offset_h])

            gt_localFeatures_new = []
            for i in range(len(valid_inds)):
                if valid_inds[i]:#True
                    one_instance = is_inside_and_conunt_localFeature(gt_localFeatures[i], img_shape[:2])
                    #if len(one_instance) == 0:
                    if len(one_instance) < 4:
                        valid_inds[i] = False
                        continue

                    # flag_up = 0
                    # if len(one_instance) < 32:
                    #     flag_up = 1
                    one_instance = local_feature_upsample(one_instance, self.num_sketchPoints) ##TODO: change the number of points
                    # !!8 points
                    # new_one_instance = []
                    # new_one_instance.append(one_instance[0])
                    # new_one_instance.append(one_instance[1])
                    # for i in range(6,len(one_instance)):
                    #     if (int(i/2))%2 == 1:
                    #         new_one_instance.append(one_instance[i])
                    #--------------------
                    # # 4points
                    new_one_instance = []
                    new_one_instance.append(one_instance[0])
                    ...
                    # new_one_instance.append(one_instance[1])
                    # # new_one_instance.append(one_instance[-1])
                    # # new_one_instance.append(one_instance[-2])

                    #--------------------------

                    # one_instance_x = one_instance[0::2]
                    # one_instance_y = one_instance[1::2]
                    # if ((one_instance_x==0).sum() > 4) & ((one_instance_x==0).sum() < 16) & (flag_up==1):
                    #     print('liye debug!!!')

                    # if (one_instance is None) or (np.isinf(one_instance).max()+np.isnan(one_instance).max()>0):
                    #     #print('wrong')
                    #     valid_inds[i] = False
                    #     continue
                    #gt_localFeatures_new.append(one_instance)

                    #shuffle
                    # pairs = list(zip(new_one_instance[0::2],new_one_instance[1::2]))
                    # random.shuffle(pairs)
                    # new_one_instance = [item for pair in pairs for item in pair]
                    # gt_localFeatures_new.append(new_one_instance)
                    #____________________________________________
                    # pairs = list(zip(one_instance[0::2],one_instance[1::2]))
                    # random.shuffle(pairs)
                    # new_one_instance = [item for pair in pairs for item in pair]
                    gt_localFeatures_new.append(new_one_instance)
            if len(gt_localFeatures_new) == 0:#tenghui
                return None
            results['gt_SketchPoints'] = gt_localFeatures_new
            results['gt_bboxes'] = bboxes[valid_inds]#renew valid_inds
 #------
            if results.get('gt_ignore_flags', None) is not None:
                results['gt_ignore_flags'] = \
       ...
       #      drawliney = centerline[1::2]
       #      for i in range(len(drawlinex)):
       #          showimg = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 255, 0), -1)
       #  cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/crop'+results['img_path'].split('/')[-1], showimg)
        return results
...
class CachedMixUp(BaseTransform):
    ...
    def __repr__(self):
        ...
        return repr_str
# tenghui
def flip_localFeature(localFeatures,
    ...
    flipped = localFeatures.clone()

    if direction == 'horizontal':
        flipped[..., 0::2] = img_shape[1] - localFeatures[..., 0::2]  # x

    elif direction == 'vertical':
        flipped[..., 1::2] = img_shape[0] - localFeatures[..., 1::2]  # y

    else:
        flipped[..., 0::2] = img_shape[1] - localFeatures[..., 0::2]
        flipped[..., 1::2] = img_shape[0] - localFeatures[..., 1::2]
    return flipped


# tenghui
def translate_localFeature(localFeatures,  distances: Tuple[float, float]) -> None:
    ...
    return localFeatures + localFeatures.new_tensor(distances).repeat(int(localFeatures.shape[-1] / 2))
def clip_localFeature(localFeatures, img_shape: Tuple[int, int]) -> None:
    ...
    localFeatures[..., 0::2] = localFeatures[..., 0::2].clamp(0, img_shape[1])
    localFeatures[..., 1::2] = localFeatures[..., 1::2].clamp(0, img_shape[0])
    return localFeatures

def is_inside_localFeature(localFeatures,
              img_shape: Tuple[int, int],
    ...
    index = torch.tensor(np.ones((localFeatures.shape[0], localFeatures.shape[1]), dtype=bool))
    # if all_inside:
    #     for i in range(localFeatures.shape[-1]):
    #         if i%2 == 0:
    #             index[:, i]  = index[:, i]  & (localFeatures[:, i] >= -allowed_border) &(localFeatures[:, i] < img_w + allowed_border)
    #         else:
    #             index[:, i]  = index[:, i]  & (localFeatures[:, i] >= -allowed_border) & (localFeatures[:, i] < img_h + allowed_border)
    # else:
    #     for i in range(localFeatures.shape[-1]):
    #         if i % 2 == 0:
    #             index[:, i]  = index[:, i]  & (localFeatures[:, i] >-allowed_border) & (localFeatures[:, i] < img_w + allowed_border)
    ...
    for i in range(localFeatures.shape[-1]):
        if i % 2 == 0:
            index[:, i] = index[:, i] & (localFeatures[:, i] >= -allowed_border) & (
                        localFeatures[:, i] < img_w + allowed_border)
        else:
            index[:, i] = index[:, i] & (localFeatures[:, i] >= -allowed_border) & (
                        localFeatures[:, i] < img_h + allowed_border)
    return index
...
#                         localFeatures[:, i] < img_h + allowed_border)
#     return index
def is_inside_and_conunt_localFeature(localFeatures,
              img_shape: Tuple[int, int],
              all_inside: bool = False,
              allowed_border: int = 0) -> BoolTensor:
    """Find boxes inside the image.
    ...
    """
    img_h, img_w = img_shape
    index = torch.tensor(np.ones(localFeatures.shape[-1], dtype=bool))
    one_instance = []
    # if all_inside:
    #     for i in range(localFeatures.shape[-1]):
    #         if i%2 == 0:
    ...
    for i in range(localFeatures.shape[-1]):
        if i % 2 == 0:
            ...
            index[i] = index[i] & (localFeatures[i] >= -allowed_border) & (
                        localFeatures[i] < img_h + allowed_border)
            if (index[i] == True) & (index[i - 1] == True):
                one_instance.append(localFeatures[i-1])
                one_instance.append(localFeatures[i])
    return one_instance
def rescale_localFeature(localFeatures, scale_factor: Tuple[float, float]) -> None:
    ...
    scale_factor = localFeatures.new_tensor(scale_factor).repeat(int(localFeatures.shape[-1]/2))
    return localFeatures * scale_factor

##liye
def local_feature_upsample(localFeatures_pred, localFeatures_outSize):
    ## get x, y
    localFeatures_pred = torch.tensor(localFeatures_pred)
    localFeatures_pred_size = int(localFeatures_pred.shape[-1]/2)
    # if localFeatures_pred_size == 1:
    #     return localFeatures_pred.repeat(localFeatures_outSize)
    if localFeatures_pred_size < 2:
        return None
    elif localFeatures_pred_size == localFeatures_outSize:
    ...
    localFeatures_pred_x_diff = torch.diff(localFeatures_pred_x)
    localFeatures_pred_y_diff = torch.diff(localFeatures_pred_y)
    localFeatures_pred_slope = torch.div(localFeatures_pred_y_diff, localFeatures_pred_x_diff)
    ...
    localFeatures_pred_y_add_list = []
    for i in range(upsample_interval):
        localFeatures_pred_x_add = localFeatures_pred_x[0:-1] + localFeatures_pred_x_diff * (
                i + 1) / (upsample_interval + 1)
    ...
    ## put all points in order
    localFeatures_pred_up_x = torch.zeros(int(localFeatures_outSize+num_deletePoints))
    localFeatures_pred_up_y = torch.zeros(int(localFeatures_outSize+num_deletePoints))
