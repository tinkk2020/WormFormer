...
#tenghui, for multi-scale local features
@TRANSFORMS.register_module()
class LoadAnnotations_localFeature(MMCV_LoadAnnotations):
    """Load and process the ``instances`` and ``seg_map`` annotation provided
    by dataset.
    ...
    def __init__(
            ...
            ignore_index: int = 255,
            **kwargs) -> None:
        super(LoadAnnotations_localFeature, self).__init__(**kwargs)
        self.with_mask = with_mask
        self.poly2mask = poly2mask
        self.box_type = box_type
        self.reduce_zero_label = reduce_zero_label
    ...
    def _load_bboxes(self, results: dict) -> None:
        ...
        for instance in results.get('instances', []):
            gt_bboxes.append(instance['bbox'])
            gt_ignore_flags.append(instance['ignore_flag'])
            gt_localFeatures.append(instance['localFeature'])
    ...
    def transform(self, results: dict) -> dict:
        ...
        # cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/load32.png', showimg)
        return results
...
#liye, for sketch point head
@TRANSFORMS.register_module()
class LoadAnnotations_SketchPoints(MMCV_LoadAnnotations):
    """Load and process the ``instances`` and ``seg_map`` annotation provided
    by dataset.
    ...
    def __init__(
            ...
            ignore_index: int = 255,
            **kwargs) -> None:
        super(LoadAnnotations_SketchPoints, self).__init__(**kwargs)
        self.with_mask = with_mask
        self.poly2mask = poly2mask
        self.box_type = box_type
        self.reduce_zero_label = reduce_zero_label
    ...
    def _load_bboxes(self, results: dict) -> None:
        ...
        for instance in results.get('instances', []):
            gt_bboxes.append(instance['bbox'])
            gt_ignore_flags.append(instance['ignore_flag'])
            gt_localFeatures.append(instance['SketchPoints'])
        ##TODO: please change hinput!!
        gt_localFeatures = np.array(gt_localFeatures, dtype=np.float32)
    ...
    def transform(self, results: dict) -> dict:
        ...
        #     for i in range(len(drawlinex)):
        #         img = cv2.circle(showimg, (int(drawlinex[i]), int(drawliney[i])), 1, (0, 0, 255), -1)
        # cv2.imwrite('/disk1/liye/mmdetection-main/data/fortest/load'+results['img_path'].split('/')[-1], showimg)
        return results
...
#tenghui
@TRANSFORMS.register_module()
class FilterAnnotations_localFeature(BaseTransform):
    """Filter invalid annotations.
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        for (keep_,gt_localFeature) in zip(keep,results['gt_localFeatures']):
            if keep_:
                gt_localFeatures_list.append(gt_localFeature)
        results['gt_localFeatures'] = gt_localFeatures_list
...
@TRANSFORMS.register_module()
class FilterAnnotations_SketchPoints(BaseTransform):
    """Filter invalid annotations.
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        ##liye
        results['gt_SketchPoints'] = torch.tensor(np.array(results['gt_SketchPoints']))[keep]
