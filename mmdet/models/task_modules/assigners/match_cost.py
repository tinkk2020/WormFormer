...
##liye, for local features
@TASK_UTILS.register_module()
class SketchL1Cost(BaseMatchCost):
    """BBoxL1Cost.
    ...
    def __call__(self,
        ...
        Returns:
            Tensor: Match Cost matrix of shape (num_preds, num_gts).
        """
        pred_localFeatures = pred_instances.localFeatures
        gt_localFeatures = gt_instances.localFeatures
        gt_localFeatures = gt_localFeatures.cuda()
        ...
        # pred_localFeatures = pred_localFeatures / factor

        localFeatures_cost = torch.cdist(pred_localFeatures, gt_localFeatures, p=1)
        return localFeatures_cost * self.weight