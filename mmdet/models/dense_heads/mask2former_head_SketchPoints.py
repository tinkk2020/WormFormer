...
class Mask2FormerHead_SketchPoints(MaskFormerHead):
    ...
    def __init__(self,
                 ...
                 sketchPoints_transformer_decoder: ConfigType = ..., #liye
                 sketchPoints_out_channels: int = 64,
                 positional_encoding: ConfigType = dict(
                     num_feats=128, normalize=True),
                 sketchPoints_positional_encoding: ConfigType = dict(
        ...
        ##liye, head for sketch points
        localFeatures_embed_inputSize = 32 ## size of the input feature map
        localFeatures_outSize = sketchPoints_out_channels## num of the sketch points
        ...
        self.locaFeatures_pred_first = nn.Sequential(
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, localFeatures_outSize))##from (batch_size, num_queries, channel) to (batch_size, num_queries, out)

        self.locaFeatures_pred = nn.Sequential(
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, localFeatures_outSize))##from (batch_size, num_queries, channel) to (batch_size, num_queries, out)

        #tenghui new transforemer_decoder
        self.num_sketchPoints_transformer_decoder_layers = sketchPoints_transformer_decoder.num_layers
        self.sketchPoints_transformer_decoder = Mask2FormerTransformerDecoder(
            **sketchPoints_transformer_decoder)

        self.loss_localFeatures = MODELS.build(loss_SketchPoints)
        self.localFeatures_positional_encoding = SinePositionalEncoding(**sketchPoints_positional_encoding)
    ...
    def preprocess_gt(
        ...
        gt_localFeatures_list = [
            gt_instances['SketchPoints'] for gt_instances in batch_gt_instances
        ]
    ...
    def _get_targets_single(self, cls_score: Tensor, mask_pred: Tensor,
                            ...
                            img_meta: dict, localFeatures_pred: Tensor) -> Tuple[Tensor]:
        ...
        gt_localFeatures = gt_instances.localFeatures  ##init gt for local features
    ...
    def get_targets(
        ...
        results = multi_apply(self._get_targets_single, cls_scores_list,
                              ...
                              batch_img_metas, localFeatures_preds_list)
    ...
    def loss_by_feat(self, all_cls_scores: Tensor, all_mask_preds: Tensor,
                     ...
                     batch_img_metas: List[dict], all_localFeatures_preds: Tensor) -> Dict[
        str, Tensor]:
        """Loss function.
        ...
        losses_cls, losses_mask, losses_dice, losses_localFeature = multi_apply(
            ...
            batch_gt_instances_list, img_metas_list, all_localFeatures_preds)
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor,
                             batch_gt_instances: List[InstanceData],
                             batch_img_metas: List[dict], localFeatures_preds: Tensor) -> Tuple[
        Tensor]:
    ...
    def local_feature_upsample(self, localFeatures_pred, localFeatures_outSize):
        ...
        for i in range(upsample_interval):
            ...
            localFeatures_pred_y_add = torch.where(torch.isinf(localFeatures_pred_slope) | torch.isnan(localFeatures_pred_slope),
    ...
    def _forward_head(self, decoder_out: Tensor, mask_feature: Tensor, attn_mask_target_size: Tuple[int, int],
                      sketchPoints_query: Tensor, sketchPoints_query_embed: Tensor, decoder_featureMap: Tensor,
                      decoder_positional_encodings: Tensor, attn_mask_pre: Tensor) -> Tuple[Tensor]:
        ...
        ##liye, predict sketch points
        sketchPoints_pred = self.locaFeatures_pred(decoder_out)

        for i in range(self.num_sketchPoints_transformer_decoder_layers):
            # cross_attn + self_attn
            layer = self.sketchPoints_transformer_decoder.layers[i]

            sketchPoints_query = layer(
                query=sketchPoints_query.cuda(),
                key=decoder_featureMap,
                value=decoder_featureMap,
                query_pos=sketchPoints_query_embed.cuda(),
                key_pos=decoder_positional_encodings,  # decoder_positional_encodings[level_idx]
                attn_masks=attn_mask_pre,
                query_key_padding_mask=None,
                key_padding_mask=None)

        sketchPoints_query = self.sketchPoints_transformer_decoder.post_norm(sketchPoints_query)
        # shape (batch_size, num_queries, c)
        mask_embed = self.mask_embed(sketchPoints_query)
        ...
        return cls_pred, mask_pred, attn_mask, sketchPoints_pred
    ...
    def forward(self, x: List[Tensor],
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            #liye, for first query from sketch points
            if i==0:
        ...
        ##liye, first-level sketch point predicting
        sketchPoints_pred_list = []
        ...
        sketchPoints_pred = self.locaFeatures_pred_first(query_feat_first.permute(0, 2, 1))
        sketchPoints_query = self.local_feature_upsample(sketchPoints_pred, self.feat_channels)
        ##liye, positional encoding for local features
        localFeatures_mask_pe= sketchPoints_query.new_zeros(
            (batch_size,1,sketchPoints_query.shape[-1]),
            dtype=torch.bool)
        sketchPoints_query_embed = self.localFeatures_positional_encoding(
            localFeatures_mask_pe)
        sketchPoints_query_embed = sketchPoints_query_embed.flatten(2)


        cls_pred_list = []
        mask_pred_list = []
        attn_mask = None ##TODO: none?
        level_idx = 0
        cls_pred, mask_pred, attn_mask, sketchPoints_pred = \
            self._forward_head(query_feat, mask_features, multi_scale_memorys[0].shape[-2:], sketchPoints_query,
                               sketchPoints_query_embed, decoder_inputs[level_idx],
                               decoder_positional_encodings[level_idx], attn_mask)
        cls_pred_list.append(cls_pred)
        mask_pred_list.append(mask_pred)
        sketchPoints_pred_list.append(sketchPoints_pred)##TODO: save the previous sketch point prediction?
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            query_feat = layer(
                ...
                key_padding_mask=None)
            cls_pred, mask_pred, attn_mask, sketchPoints_pred = self._forward_head(
                query_feat, mask_features, multi_scale_memorys[(i + 1) % self.num_transformer_feat_level].shape[-2:],
                sketchPoints_query, sketchPoints_query_embed, decoder_inputs[level_idx],
                decoder_positional_encodings[level_idx], attn_mask)
            sketchPoints_query = self.local_feature_upsample(sketchPoints_pred, self.feat_channels)

            cls_pred_list.append(cls_pred)
            mask_pred_list.append(mask_pred)
            sketchPoints_pred_list.append(sketchPoints_pred)

        return cls_pred_list, mask_pred_list, sketchPoints_pred_list
    ...
    def loss(
        ...
        # forward
        all_cls_scores, all_mask_preds, sketchPoints_preds = self(x, batch_data_samples)
        ...
        losses = self.loss_by_feat(all_cls_scores, all_mask_preds,
                                   batch_gt_instances, batch_img_metas, sketchPoints_preds)
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        all_cls_scores, all_mask_preds, all_sketchPoints_preds = self(x, batch_data_samples)
        mask_cls_results = all_cls_scores[-1]
        mask_pred_results = all_mask_preds[-1]
        sketchPoints_preds_results = all_sketchPoints_preds[-1]
