...
def preprocess_panoptic_gt_AddParts(gt_labels: Tensor, gt_masks: Tensor,
                           gt_semantic_seg: Tensor, num_things: int,
                           num_stuff: int, gt_localFeatures: Tensor) -> Tuple[Tensor, Tensor]:
    """Preprocess the ground truth for a image.
    ...
    if gt_semantic_seg is None:
        masks = things_masks.long()
        return gt_labels, masks, gt_localFeatures

    things_labels = gt_labels
    gt_semantic_seg = gt_semantic_seg.squeeze(0)

    ##liye, local features
    things_localFeatures = gt_localFeatures
    ...
    if len(stuff_masks_list) > 0:
        ...
        labels = things_labels
        masks = things_masks
        localFeatures = things_localFeatures ##local features

    masks = masks.long()

    ##liye, local features
    localFeatures = localFeatures.long()

    return labels, masks, localFeatures