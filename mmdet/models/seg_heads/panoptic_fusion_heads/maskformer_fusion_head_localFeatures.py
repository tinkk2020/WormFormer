...
class MaskFormerFusionHead_LocalFeatures(BasePanopticFusionHead):
    ...
    def panoptic_postprocess(self, mask_cls: Tensor,
                             mask_pred: Tensor, localFeature_pred: Tensor) -> PixelData:
    ...
    def instance_postprocess(self, mask_cls: Tensor,
                             mask_pred: Tensor, localFeature_pred: Tensor, meta: dict, rescale: bool) -> InstanceData:
        ...
        localFeature_pred = localFeature_pred[query_indices]##liye
        ...
        localFeature_pred = localFeature_pred[is_thing]
        ...
        factor = localFeature_pred.new_tensor([img_w, img_h]).repeat(int(localFeature_pred.shape[-1]/2)).unsqueeze(0)
        localFeature_pred = localFeature_pred * factor
        ##liye, output results in original resolution
        if rescale:
            ori_height, ori_width = meta['ori_shape'][:2]
            scale_factor = [ori_width/img_w, ori_height/img_h]
            scale_factor = localFeature_pred.new_tensor(scale_factor).repeat(int(localFeature_pred.shape[-1]/2)).unsqueeze(0)
            localFeature_pred = localFeature_pred * scale_factor

        # localFeature_pred = torch.tensor(localFeature_pred,dtype=torch.int32)
        ...
        results.localFeatures = localFeature_pred
        return results

    def predict(self,
                mask_cls_results: Tensor,
                mask_pred_results: Tensor,
                localFeature_pred_results: Tensor,
        ...
        results = []
        for mask_cls_result, mask_pred_result, localFeature_pred_result, meta in zip(
                mask_cls_results, mask_pred_results, localFeature_pred_results, batch_img_metas):
            # remove padding
            ...
            if panoptic_on:
                pan_results = self.panoptic_postprocess(
                    mask_cls_result, mask_pred_result, localFeature_pred_result)
                result['pan_results'] = pan_results

            if instance_on:
                ins_results = self.instance_postprocess(
                    mask_cls_result, mask_pred_result, localFeature_pred_result, meta, rescale)
